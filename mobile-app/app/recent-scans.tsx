...
import { fetchHomeRecentScans, HomeRecentScanItem } from "../src/api/home.api";
...
export default function RecentScansScreen() {
  ...
  const [items, setItems] = useState<HomeRecentScanItem[]>([]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const data = await fetchHomeRecentScans(30, 20);
        if (mounted) setItems(data.items || []);
      } catch (err: any) {
        if (mounted) setError(err?.response?.data?.message || "Impossible de charger les scans.");
      } finally {
    ...
    <Screen>
      <ScrollView contentContainerStyle={styles.content}>
        ...
        {!loading && !error && items.length === 0 ? (
          <Text style={styles.emptyText}>Aucun scan sur la periode.</Text>
        ) : null}

        {items.map((item) => (
          <View key={item.id} style={pageStyles.card}>
            <Text style={pageStyles.cardTitle}>{item.plateNumber}</Text>
            <Text style={styles.meta}>Modele: {item.modelUsed}</Text>
            <Text style={styles.desc}>
              Vehicule: {item.vehicle ? `${item.vehicle.brand} ${item.vehicle.model}` : "Non trouve"}
            </Text>
          </View>
        ))}
